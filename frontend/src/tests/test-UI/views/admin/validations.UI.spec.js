...
describe("Validations.vue - Tests UI et integration", () => {
  ...
  it("filtre dynamiquement les validations selon la recherche", async () => {
    const wrapper = await mountValidations();

    expect(wrapper.vm.filteredValidations).toHaveLength(4);

    wrapper.vm.search = "Yassine";
    await wrapper.vm.$nextTick();

    expect(wrapper.vm.filteredValidations).toHaveLength(1);
    expect(wrapper.vm.filteredValidations[0].student.fullName).toBe("Yassine K.");
  });

  it("ouvre la modale de details lors de l'evenement view", async () => {
    const wrapper = await mountValidations();
